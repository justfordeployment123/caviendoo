...
const RECOLTES_SWATCHES = [
  { color: '#0a1210', label: '0' },
  { color: '#164a28', label: '1–6' },
  { color: '#208040', label: '7–12' },
  { color: '#3cb858', label: '13–19' },
  { color: '#78f090', label: '20+' },
];
...
export function MapLegend({ overlayMode }: MapLegendProps) {
  ...
  return (
    <div className="absolute bottom-4 start-4 z-10 bg-surface/90 border border-border rounded px-3 py-2.5 backdrop-blur-sm">
      ...
      {overlayMode === 'stress-hydrique' && (
        <div>
          ...
          <div className="flex justify-between w-28 sm:w-36 mt-0.5">
            <span className="text-2xs text-[#86efac]">{t('legendSustainable')}</span>
            <span className="text-2xs text-[#fca5a5]">{t('legendCritical')}</span>
          </div>
        </div>
      )}
      ...
      {overlayMode === 'indice-uv' && (
        <div>
          ...
          <div className="flex justify-between w-28 sm:w-36 mt-0.5">
            <span className="text-2xs text-[#f5f0c0]">{t('legendLow')}</span>
            <span className="text-2xs text-[#fca5a5]">{t('legendExtreme')}</span>
          </div>
        </div>
      )}
    </div>
